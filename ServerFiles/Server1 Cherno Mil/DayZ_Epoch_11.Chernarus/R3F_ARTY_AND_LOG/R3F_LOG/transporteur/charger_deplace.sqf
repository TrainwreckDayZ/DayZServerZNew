...
else
{
	...
	if (count _transporteur > 0) then
	{
		...
		_vehPos = [_transporteur] call FNC_GetPos;
		if (alive _transporteur && ([0,0,0] distance velocity _transporteur < 6) && ((_vehPos select 2) < 2) && !(_transporteur getVariable "R3F_LOG_disabled")) then
		{
