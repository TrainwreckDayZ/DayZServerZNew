...
_repair_costs = [];
_rearm_enable = true;
_rearm_costs = ["ItemBriefcase100oz",1]; //"ItemBriefcase100oz",1
...
_fnc_actionTitle = {
	...
	if (1 in _type) then {_costsText = _costsFree;};
...
while {1 == 1} do {
	...
	if (local _vehicle && _inVehicle) then {
		...
		_inRange = ({alive _x} count _objects) > 0;
		if (_inRange) then {
			{
				switch (true) do {
					case (getNumber(configFile >> "cfgVehicles" >> typeOf(_x) >> "transportAmmo") >= 100000): {_type set [(Count _type), 1];};
					case (getNumber(configFile >> "cfgVehicles" >> typeOf(_x) >> "transportFuel") >= 3000): {_type set [(Count _type), 2];};
					case (getNumber(configFile >> "cfgVehicles" >> typeOf(_x) >> "transportRepair") > 0): {_type set [(Count _type), 3];};
					default {_type set [(Count _type), 0];};
				};
			} count _objects;
			...
			if (s_player_refuel_action < 0 && {_refuel_enable} && {(0 in _type) || (2 in _type)}) then {
			...
			if (s_player_repair_action < 0 && {_repair_enable} && {(0 in _type) || (3 in _type)}) then {
			...
			if ((((count _role) > 1) || ((driver _vehicle) == player)) && {_rearm_enable} && {s_player_rearm_action < 0} && {(0 in _type) || (1 in _type)}) then {
					...
					if ((driver _vehicle) == player) then {
						...
						_testReturn = _vehWep call _testMyReturn;
							if ((Count _testReturn) > 0) then {
								s_player_rearm_action = _vehicle addAction [_actionTitle, "service_point\service_point_rearm.sqf", [_rearm_costs, _allInfo, _type], -1, false, true, "", _actionCondition];
							};
						} else {
							s_player_rearm_action = _vehicle addAction [_actionTitle, "service_point\service_point_rearm.sqf", [_rearm_costs, _allInfo, _type], -1, false, true, "", _actionCondition];
					};
