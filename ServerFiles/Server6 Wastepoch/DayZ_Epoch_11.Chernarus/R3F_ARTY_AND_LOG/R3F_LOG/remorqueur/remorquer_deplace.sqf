if (R3F_LOG_mutex_local_verrou) then
{
	...
	if (count _remorqueur > 0) then
	{
		...
		if (alive _remorqueur && isNull (_remorqueur getVariable "R3F_LOG_remorque") && ([0,0,0] distance velocity _remorqueur < 6) && (([_remorqueur] call FNC_GetPos) select 2 < 2) && !(_remorqueur getVariable "R3F_LOG_disabled")) then
		{
